import {
  ...
  Context,
  UpdateIntersection,
} from "https://deno.land/x/mtkruto@0.2.40/mod.ts";
import {
  MiddlewareFn,
  MiddlewareObj,
} from "https://deno.land/x/mtkruto@0.2.40/client/1_composer.ts";
...
export class Analytics extends AnalyticsDispatcher implements MiddlewareObj<T> {
...
function constructEntry(ctx: T): Entry | null {
  ...
  if (ctx.msg?.forwardFrom) {
    forwardFrom = ctx.msg.forwardFrom.id;
  } else if (ctx.msg?.forwardFromChat) {
    forwardFrom = ctx.msg.forwardFromChat.id;
  }

  let forwardName: string | undefined;
  if (ctx.msg?.forwardFrom) {
    forwardName = ctx.msg.forwardFrom.firstName;
    if (ctx.msg.forwardFrom.lastName) {
      forwardName += " ";
      forwardName += ctx.msg.forwardFrom.lastName;
      forwardName = forwardName.trim();
    }
  } else if (ctx.msg?.forwardFromChat) {
    forwardName = "title" in ctx.msg.forwardFromChat
      ? ctx.msg.forwardFromChat.title
      : ctx.msg.forwardFromChat.firstName;
    if (
      "lastName" in ctx.msg.forwardFromChat && ctx.msg.forwardFromChat.lastName
    ) {
      forwardName += " ";
      forwardName += ctx.msg.forwardFromChat.lastName;
      forwardName = forwardName.trim();
    }
  } else if (ctx.msg?.forwardSenderName) {
    forwardName = ctx.msg.forwardSenderName;
  }
  ...
  const forward_date = ctx.msg?.forwardDate ?? new Date();
  const forward_from = forwardFrom ?? 0;
  const forward_messageid = ctx.msg?.forwardId ?? 0;
  const forward_signature = ctx.msg?.forwardSignature ?? "";
  const forward_bot = ctx.msg?.forwardFrom?.isBot ?? false;
